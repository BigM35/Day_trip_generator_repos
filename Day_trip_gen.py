...
things_to_do = [" going on a free walking tour", "taking an audio tour", "exploring the Markets", "visiting Free Museums and Galleries", "hanging out with the locals", "going on a Carnival, then to a festival and enjoying some free live performances", "Strolling around a Park", "going to the library", "visiting the Churches and Cathedrals and asking for forgivness"]
...
def determine_user_input(pass_input):
    while "no" != pass_input.lower() != 'yes':
        print("\n")
        pass_input = input("Invalid Entry! Please answer with [Yes/No]. So, are you satified with the previous recommendation? [Yes/No]: ").lower()
...
def introduction():
    ...
    if "no" == checked_answer:
        ...
        print("\n")
        print(f"Excellent choice {user_name}! lets start off with the destination...")
...
def generate_new_choice(current_list):
    ...
    while checked_user_answer == "no":
        print("\n")
        user_new_choice = random.choice(current_list)
        user_choice = user_new_choice
        user_answer = input(f"How about {user_choice}. Sounds good? [Yes/No]: ").lower()
        checked_user_answer = determine_user_input(user_answer)
...
#Check and make sure both restaurants are the same
def verfied_restaurant(first_des, final_des, curr_restaurant_details):
    if first_des != final_des:
        return get_restaurant(final_des)
    else:
        return curr_restaurant_details

#Check if user is satified with their decisions so far
def user_confirmation(curr_des_name_so_far, listt, details):
    print("\n")
    user_confirmation = input(f"Are you satified with {curr_des_name_so_far}? [Yes/No]: ")
...
#call the confirmation function to verify that user is satified with choices so far
new_destination = user_confirmation(destination_name, cities, destination_details)
new_destination_name = get_city_name(new_destination)
new_restaurant = verfied_restaurant(destination_name, new_destination_name, chosen_restaurant)
new_thing_to_do = user_confirmation(chosen_thing_to_do, things_to_do, chosen_thing_to_do)
new_transport = user_confirmation(chosen_transport, mode_of_transportation, chosen_transport)
verfy_new_restaurant = user_confirmation(new_restaurant, new_destination_name, chosen_restaurant)

#outro/ confirm trip
print("\n") 
final_confirmation = input(f"Congratulations {user_name}! You have completed all choices needed to generate the day trip of your dreams. You will be {new_transport}to you newly generated day trip to {new_destination_name}, which you will be {new_thing_to_do}. After a long day of {new_thing_to_do}, you will be dining at the fabulous restaurant called {verfy_new_restaurant}. That should conclude your trip to {new_destination_name}, at which point, you will be {new_transport} back home. Does this sound acceptable for you? [Yes/No]: ")
...
if determine_user_input(final_confirmation) == ("yes"):
    ...
    while determine_user_input(final_confirmation) == ("no"):
        print("\n")
        print("Okay, Lets back up a little bit and re-confirm what you are satified/ not satified with. ")
        new_destination = user_confirmation(new_destination_name , cities, new_destination)
        new_destination_name = get_city_name(new_destination)
        new_restaurant = verfied_restaurant(destination_name, new_destination_name, chosen_restaurant)
        new_thing_to_do = user_confirmation(chosen_thing_to_do, things_to_do, chosen_thing_to_do)
        new_transport = user_confirmation(chosen_transport, mode_of_transportation, chosen_transport)
        ...
        print("\n") 
        final_confirmation = input(f"Congratulations {user_name}! You have completed all choices needed to generate the day trip of your dreams. You will be {new_transport} to your newly generated day trip to {new_destination_name}, which you will be {new_thing_to_do}. After a long day of {new_thing_to_do}, you will be dining at the fabulous restaurant called {verfy_new_restaurant}. That should conclude your trip to {new_destination_name}, at which point, you will be {new_transport}back home. Does this sound acceptable for you? [Yes/No]: ")
